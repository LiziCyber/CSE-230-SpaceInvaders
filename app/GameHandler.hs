...
moveHorizontal f g = if stopped g then g
  else g {canon = V2 x $ canon g ^._y, lives = new_lives}
    where x         = if V2 new_x (canon g ^._y) `elem` [bCoord blocker | blocker <- blockers g] then old_x else new_x
          old_x     = canon g ^._x
          new_x     = f old_x `mod` width
          new_lives = if V2 x (canon g ^._y) `elem` [coord alien | alien <- aliens g] then max 0 (lives g - 1) else lives g

-- | Move the canon up or down
moveVertical :: (Int -> Int) -> Game -> Game
moveVertical f g = if stopped g then g
  else g {canon = V2 (canon g ^._x) y, lives = new_lives}  
    where y 
            | new_y < 0 = 0
            | new_y >= height = height - 1
            | otherwise = if V2 (canon g ^._x) new_y `elem` [bCoord blocker | blocker <- blockers g] then old_y else new_y
          old_y = canon g ^._y
          new_y = f old_y
          new_lives = if V2 (canon g ^._x) y `elem` [coord alien | alien <- aliens g] then max 0 (lives g - 1) else lives g
...
moveAndKill a s = [x | x <- a', 0 /= hits x] 
  where a' = map (\(Alien c h t) -> if c `elem` s then Alien c (h - hitReduction t) t else Alien c h t) a
...
handleScore g s = score g + sum (map alienScore h) + length u * 10
  where h = [x | x <- aliens g, coord x `elem` s]
...
handleLives g ashots as
      | a = 0
      | is_collision = lives g - 1
      | null h = lives g
      | otherwise = lives g - 1
            where h = [x | x <- ashots, x == canon g] --canon is hit
                  a = any (\c -> c ^._y <= 1) (alientLocations g) --game over when aliens at bottom
                  is_collision = count g == 0 && canon g `elem` [coord alien | alien <- as]
