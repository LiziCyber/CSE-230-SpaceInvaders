...
drawUI (GameState g) = [ C.center $ padRight (Pad 2) (drawStats g) <+> drawGrid g ]
drawUI MenuState = [ C.center $
                    vBox
                      [ str "SPACE INVADERS" <=> str " "
                      , B.borderWithLabel (str "") (str "Start Game (Press Enter)") <=> B.borderWithLabel (str "") (str "Leaderboard (Press L)") <=> B.borderWithLabel (str "") (str "Quit (Press Esc)")
                      ]
                  ]
drawUI (LeaderboardState l) = [ C.center $ T.renderTable $ T.table $ drawTable (slice low high (records l)) ranks]
  where low   = startIdx l
...
drawTable records' ranks = drawTable [] [-1] ++ zipWith drawRow records' ranks
  where
    drawRow :: LeaderboardRecord -> Int -> [Widget Name]
    drawRow record rank = [str (show rank), str (name record), str (show (score' record)), str (show (time record))]
...
drawStats g = hLimit 18
  $ vBox [ drawStatus $ status g
         ...
         , drawInfoBox " Info "  $ str "Fire:    space" <=> str "Pause:   p    " <=> str "Restart: r    " <=> str "Back:    esc  "
         ]
...
drawCell NewAlienCell = withAttr newAlienAttr $ str "ΩΩΩ" <=> str "πππ"
drawCell NewAlienHitOnceCell = withAttr newAlienAttr $ str "XXX" <=> str "πππ"
